...
class PemerintahanPKKDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        $actionBtn .= '<div class="col">
        <a href="' . route('pemerintahan-pkk.index') . '/{{ $id }}" name="view" class="btn btn-primary mt-2"><i class="fa-solid fa-eye"></i></a>';
            ...
            ->filter(function ($query) {
                if (request()->has('search') && !empty(request()->get('search')['value'])) {
                    ...
                    $query->where(function ($q) use ($search) {
                        $q->whereRaw('LOWER(pemerintahan_pkk.nama) LIKE ?', ["%{$search}%"])
                            ->orWhereRaw('LOWER(pemerintahan_pkk.jabatan) LIKE ?', ["%{$search}%"]);
                    });
                }
            })
    ...
    public function query(PemerintahanPKK $model): QueryBuilder
    {
        return $model->newQuery()
            ->select(
                ...
                'pemerintahan_pkk.created_at as created_at',
            ...
            ->join('users', 'users.id', '=', 'pemerintahan_pkk.user_id')
